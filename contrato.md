...
- **Responsividad Móvil (max-width: 1023px):**
  ...
  - En mobile, `#dashboard-body` cambia a `flex-direction:column` con `overflow-y:scroll`. Cada contenedor de gráfico (hijo directo) ocupa ancho completo (`min-width:0`), altura mínima `120px` (`flex:1 0 auto`) y se separa con borde inferior sutil. Esto evita que 3 gráficos side-by-side se solapen en pantallas angostas, permitiendo scroll vertical dentro del dashboard.
